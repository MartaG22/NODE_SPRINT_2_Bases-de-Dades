...
db.restaurants.find({$and: [{ "cuisine": {$in: ["Seafood"], $nin: ["American", "Chinese"]}}, { "name": {$not: { $regex: /^Wil/ }}}] }, { "name": 1, "restaurant_id": 1, "borough": 1, "cuisine": 1, "_id": 0 }); //! corregida
...
db.restaurants.find({"address.street": {$exists: null}}).count();                                                       //! Corregida
db.restaurants.find({"address.coord": {$type: "double"}}).pretty;                                                     //! Corregida
db.restaurants.find({"grades.score": {$mod:[7,0]}}, {restaurant_id: 1, name: 1, grade: 1});                           //! Corregida
