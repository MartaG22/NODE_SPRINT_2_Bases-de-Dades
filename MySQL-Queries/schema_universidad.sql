...
SELECT apellido1, apellido2, persona.nombre, departamento.nombre FROM ((persona INNER JOIN profesor ON profesor.id_profesor = persona.id) INNER JOIN departamento ON departamento.id = profesor.id_departamento ORDER BY apellido1, apellido2, nombre;
...
/*SELECT departamento.nombre FROM (((departamento INNER JOIN profesor ON profesor.id_departamento = departamento.id)  INNER JOIN asignatura ON asignatura.id_profesor = profesor.id_profesor) INNER JOIN grado ON grado.id = asignatura.id_grado) WHERE grado.nombre = 'Grau en Enginyeria Informàtica (Pla 2015)'; */
-- asignatura ON asignatura.id_grado = grado.id WHERE grado.nombre = 'Grau en Enginyeria Informàtica (Pla 2015)';
...
-- B- Resol les 6 següents consultes utilitzant les clàusules LEFT JOIN i RIGHT JOIN.
...
/*             SELECT DISTINCT nombre, COUNT(DISTINCT id_profesor) FROM departamento INNER JOIN profesor ON profesor.id_departamento = departamento.id; */

-- departamento INNER JOIN profesor ON profesor.id_departamento = departamento.id);
...
/* SELECT DISTINCT grado.nombre, COUNT(DISTINCT asignatura.id_grado) FROM grado INNER JOIN asignatura ON asignatura.id_grado = grado.id  */
...
SELECT p.tipo, p.nombre, p.apellido1, p.apellido2, a.id AS asignatura, d.nombre AS departamento FROM persona p JOIN profesor pro ON p.id = pro.id_profesor JOIN departamento d ON d.id = pro.id_departamento LEFT JOIN asignatura a ON pro.id_profesor = a.id_profesor WHERE a.id is NULL;s
